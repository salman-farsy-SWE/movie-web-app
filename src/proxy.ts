...
export function proxy(request: NextRequest) {
  ...
  // Redirect /:id/list/:listId and /:id/lists/:listId to /list/:listId
  const userScopedListMatch = pathname.match(/^\/[^/]+\/lists?\/([^/?#]+)/);
  if (userScopedListMatch && userScopedListMatch[1]) {
    const listId = userScopedListMatch[1];
    const targetUrl = new URL(`/list/${listId}${search}`, request.url);
    return NextResponse.redirect(targetUrl);
  }
